...
if find . -name "*.md" -not -path "./src/vendor/*" -not -path "./node_modules/*" -not -path "./docs/*" | grep -q .; then
    if command_exists markdownlint; then
        ...
        if markdownlint "**/*.md" --ignore node_modules --ignore src/vendor --ignore docs --ignore "WORK_PLAN*.md" --ignore "SESSION_SUMMARY_*.md" --ignore "TODOs.md" --ignore "TODOs-*.md" --ignore "NEXT_STEPS*.md" --ignore "BUG_FIX_PLAN.md"; then
