...
func TestMilvusIntegration(t *testing.T) {
	...
	t.Run("GetSchema", func(t *testing.T) {
		...
		// Milvus may not preserve the exact collection name in schema
		if schema.Class != collectionName && schema.Class != "" {
			t.Logf("Note: Schema class %q differs from collection name %q", schema.Class, collectionName)
		}
	})
