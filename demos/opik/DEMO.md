...
weave docs ls OpikDemo --milvus-local
```
...
weave docs create OpikDemo \
    docs/ragme-io.pdf \
    --image-col OpikDemoImages --weaviate-cloud
```

### Ingest images (Weaviate)

```bash
weave docs create OpikDemoImages \
    docs/dog.png --weaviate-cloud

weave docs ls OpikDemoImages --weaviate-cloud
```
...
weave cols query OpikDemoImages \
...
weave stats OpikDemo --milvus-local

# Schema on weaviate-cloud
weave cols show OpikDemo \
...
weave pipeline ingest docs/vdbs/redis/ \
    --collection OpikDemo --weaviate-cloud
...
```text
> list my collections
> how many documents in OpikDemo?
> search OpikDemo for vector database architecture
```
...
weave cols del OpikDemo --force --milvus-local

# Weaviate
weave docs da OpikDemo --no-confirm --weaviate-cloud
weave docs da OpikDemoImages --no-confirm --weaviate-cloud
weave cols ds OpikDemo --no-confirm --weaviate-cloud
weave cols ds OpikDemoImages --no-confirm --weaviate-cloud
```
