...
// ListNeo4jCollections lists Neo4j collections
func ListNeo4jCollections(ctx context.Context, cfg *config.VectorDBConfig, limit int, virtual bool, jsonOutput bool) {
	client, err := CreateVectorDBClient(cfg)
	if err != nil {
		// Check if this might be a configuration error
		formattedErr := config.FormatConfigError(err)
		if formattedErr != err.Error() {
			...
			PrintError(fmt.Sprintf("Failed to create Neo4j client: %v", err))
		}
		return
	}
	...
	if err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			PrintError("Failed to connect to Neo4j: connection timeout after 30 seconds")
			fmt.Println()
			PrintInfo("Please check that:")
			PrintInfo("  1. Neo4j is running (for local) or credentials are correct (for cloud)")
			PrintInfo("  2. The URL is correct (bolt://localhost:7687 for local)")
			PrintInfo("  3. There are no network/firewall issues")
		} else {
			PrintError(fmt.Sprintf("Failed to list collections: %v", err))
		}
	...
	if len(collectionInfos) == 0 {
		if !jsonOutput {
			PrintWarning("No collections found in the Neo4j database")
		} else {
			// Output empty JSON
			fmt.Println(`{"collections": [], "total": 0}`)
		}
...
func QueryMultipleCollectionsCrossVDB(ctx context.Context, collectionSpecs []CollectionSpec, vdbConfigs map[string]*config.VectorDBConfig, queryText string, options weaviate.QueryOptions) {
	fmt.Printf("Querying %d collections across multiple VDBs...\n", len(collectionSpecs))
	...
	for _, spec := range collectionSpecs {
		...
		fmt.Printf("Querying collection '%s' from %s...\n", spec.Name, vdbKey)
		...
		fmt.Printf("Found %d results from '%s' (%s)\n", len(results), spec.Name, vdbKey)
	}
