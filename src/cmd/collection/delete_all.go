...
func runCollectionDeleteAll(cmd *cobra.Command, args []string) {
	...
	dbConfig := utils.HandleSingleDatabaseSelection(ctx, selection, cfg, "", "weave cols delete-all")
	...
	if !force {
		...
		if !utils.ConfirmActionStrict("🚨 This will permanently delete ALL documents. Type 'yes' to confirm:") {
			fmt.Println("Operation cancelled")
			return
		}
	}
